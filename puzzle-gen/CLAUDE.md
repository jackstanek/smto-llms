...
- **Output streams**: The rendered puzzle is the program's *only* stdout output and goes to `--output` (default `-` = stdout). All diagnostics — progress, intermediate state, errors — must go through the `log` crate, which `env_logger` routes to stderr. Do **not** introduce `println!` / `print!` / `eprintln!` in `src/`; they corrupt the stdout puzzle stream or bypass the configured log level. The only permitted `println!` is in `build.rs` for `cargo:` directives.
